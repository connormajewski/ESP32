...
void playRandom(fs::FS& fs, std::vector<String>& filepaths) {

  randomSeed(esp_random());
  int randomIndex = random(filepaths.size() - 1);

  String temp = filepaths[randomIndex];

  char path[64];
  
  temp.toCharArray(path, sizeof(path)); // copy into buffer

  Serial.printf("PLAYING: %s\n", path);
  playMonoWAVFile(SD_MMC, (const char *)path);
...
void setup() {
  ...
  if (!isSDInit) {
    ...
    // //tft.fillScreen(TFT_BLACK);
    // tft.fillRect(50, 50, 50, 50, TFT_BLUE);
  }
}
